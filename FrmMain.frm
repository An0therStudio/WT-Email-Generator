...
Private Sub UserForm_Activate()
    ...
    INIPath = "\\YGK01CFP01\Operations\Call Execution\WT Team\Macros & Tools\EmailGenerator\EmailGenerator.ini"
    LoadINIData
End Sub
...
Private Sub BtnGenDetailed_Click()
...
' Speaker list:
    ...
    SpeakerList = SpeakerList & "<li><span style='background-color:yellow'>Will presenters dial in together, on a single phone line, or individually?</span>" & Space(1)
    ...
    SpeakerList = SpeakerList & "<li><span style='background-color:yellow'>Will there be any additional, unlisted persons joining the speaker line?</span>" & Space(1)
    SpeakerList = SpeakerList & "</ul>"
...
' Create the Table
    ...
    EmailContent = EmailContent & EmailTable & "<br><br>"
    
' Final message:
    EmailContent = EmailContent & "If you have any questions or concerns, please do not hesitate to contact us <b>" & WTStartDay & " to " & WTEndDay & "</b> between the hours of <b>" & WTStartTime & " and " & WTEndTime & " eastern time </b> via email at " & WTEmail & " or via phone at " & WTTelNo & ".<br> If outside these hours and <i>urgent</i>, please contact us via phone at " & WTAHTelNo & ". Otherwise you may contact us via email at " & WTAHEmail & " for after-hours support.<br><br><br> Thank you and have a great day,<br><br></p>"
' Sign Off
    UserSignature = ""
...
Private Sub BtnGenShort_Click()
    ...
    EmailContent = EmailContent & " and I am contacting you regarding a conference call that you have scheduled for <b>"
    EmailContent = EmailContent & theDate & "</b> with the ID number listed above.  "
    ...
    EmailTable = EmailTable & "<span style='color:white; font-size:90%'><b>Contact Name:</b></span></td>"
    ...
    EmailTable = EmailTable & "<tr><td valign=top style='width:133.85pt;border-top:none;border-left:solid windowtext 2.25pt;border-bottom:solid windowtext 1.0pt;border-right:none;background:#4F81BD;padding:0in 5.4pt 0in 5.4pt'>"
    EmailTable = EmailTable & "<span style='color:white; font-size:90%'><b>Contact Number:</b></span></td>"
    ...
    EmailTable = EmailTable & "<span style='color:white; font-size:90%'><b>Date & Time* to Review:</b></span></td>"
    ...
    ' Compile Email body.
    HTMLBody = "<html><head><style>body {font-family:tahoma; line-height: 1; font-size:11pt}</style>"
    ...
    HTMLBody = HTMLBody & "<span style='font-size:80%'><i>*please be sure to include time zone.</i></span><br>"
...
Private Sub BtnGenAISV_Click()
    'Part Report
    'Password or Conference ID
    '35 character max
    Dim strInfo, strCompany, strLeader, strItfs, strTitle, strBody, strAltEntry As String
    Dim x As Integer
    Dim blEp, blPass, blSecure, blItfs As Boolean
    Dim var As Variant
    'Initialize
    strInfo = "AISV FOR CID #" & GetConfId() & vbCrLf & vbCrLf & vbCrLf
    strTitle = ""
    strBody = ""
    strCompany = ""
    strLeader = ""
    ...
    blItfs = False
'Call Type
    x = InStr(GetPartEntry(), "Event Plus")
    If x > 0 Then blEp = True
'Password
    If GetPassword() <> "NO PASSWORD FOUND" And blEp = False Then
        blPass = True
        blSecure = True
        var = MsgBox("Hard Password?", vbYesNo)
        If var = vbNo Then
            blSecure = False
        End If
        strInfo = strInfo & "Change Background Colour: " & blSecure & vbCrLf
    End If
'ITFS
    strItfs = RunGeneralRegex("ITFS/ITL: (\w+)", TxtDataSheet.Text)
    If strItfs = "Yes" Then blItfs = True
    strInfo = strInfo & "ITFS Bridge: " & blItfs & vbCrLf
'Company Name
    strCompany = GetCompanyName()
    strInfo = strInfo & "Company: " & strCompany & vbCrLf
    strCompany = AlphaNumericOnly(strCompany)
    'MsgBox (strCompany)
'First Leader Name
    strLeader = GetSpeakerList()
    x = 0
    x = InStr(strLeader, ",")
    If x > 0 Then strLeader = Mid(strLeader, 1, x - 1)
    strInfo = strInfo & "Leader: " & strLeader & vbCrLf & vbCrLf & vbCrLf
    'strLeader = AlphaNumericOnly(strCompany)
    'MsgBox (strLeader)
'Split Leader Name
    arrLeaderName = Split(strLeader, " ")
'Create Title
    'EP title
    If blEp Then
        strCompany = Left(strCompany, 3)
        'If UBound(arrLeaderName) <= 2 Then
            strTitle = strCompany & "\" & (arrLeaderName(UBound(arrLeaderName)))
            strTitle = UCase(strTitle)
            strInfo = strInfo & "EP Title: " & vbCrLf & strTitle & vbCrLf & vbCrLf
        'End If
    Else
    'OA title
        strTitle = strCompany & "\" & strLeader
        strTitle = UCase(strTitle)
        'MsgBox (Len(strTitle))
        strInfo = strInfo & "OA Title: " & vbCrLf & strTitle & vbCrLf
        If Len(strTitle) > 35 Then
            strInfo = strInfo & vbCrLf & "**WARNING - TITLE EXCEEDS 35 CHARACTERS**" & vbCrLf & vbCrLf
            strCompany = Mid(strCompany, 1, (35 - (Len(strLeader) + 1)))
            strTitle = strCompany & "\" & strLeader
        End If
    End If
    strTitle = UCase(strTitle)
    strInfo = strInfo & "Suggested Title: " & vbCrLf & strTitle & vbCrLf & vbCrLf & vbCrLf
    'MsgBox (strTitle)
'Create Body
    'Opening line
    strBody = vbDoubleQuote & "Good Morning/Afternoon/Evening.  May I have your " & (IIf(blPass, "password", "conference ID number")) & " please?" & vbDoubleQuote
    strBody = strBody & vbCrLf & vbCrLf & vbDoubleQuote & "May I have the spelling of your first and last name please?" & vbDoubleQuote
    'TODO: Fill in remaining FC
    'Password/ID Not Known
    strBody = strBody & vbCrLf & vbCrLf & vbCrLf & vbDoubleQuote & "If " & (IIf(blPass, "password", "ID")) & " is not known:" & vbDoubleQuote & vbCrLf
    If blPass Then
        strAltEntry = vbDoubleQuote & "May I have your conference ID number please?" & vbDoubleQuote & vbCrLf
    End If
    strAltEntry = strAltEntry & vbDoubleQuote & "May I have the name of the company hosting the conference?" & vbDoubleQuote & vbCrLf & vbDoubleQuote & "May I have the speaker's name?" & vbDoubleQuote & vbCrLf & vbCrLf & "If necessary:" & vbCrLf & vbDoubleQuote & "May I have the topic?" & vbDoubleQuote & vbCrLf & vbCrLf & "If the caller does not know the conference ID, or the company and speaker's name, or topic:" & vbCrLf & vbDoubleQuote & "I'm sorry.  For security reasons I am not able to join you to the conference.  You will need to contact someone within the company hosting the call to obtain this information." & vbDoubleQuote
    If blSecure Then
        strAltEntry = vbDoubleQuote & "I'm sorry.  For security reasons I am not able to join you to the conference.  You will need to contact someone within the company hosting the call to obtain this information." & vbDoubleQuote
    End If
    strBody = strBody & strAltEntry
    strBody = strBody & vbCrLf & vbCrLf & "Answering a Branded Speaker Line:" & vbCrLf & vbDoubleQuote & "Will you be presenting on today's conference, or are you calling in to listen to the presentation today?" & vbDoubleQuote & vbCrLf & "If they say they are a participant only: " & vbDoubleQuote & "Thank you.  I will join you to the main conference as a listen only participant." & vbDoubleQuote & vbCrLf & "If they say they are a speaker: " & vbDoubleQuote & "I will join you to the leader sub conference, as you will be presenting on today's call, correct?" & vbDoubleQuote
    'MsgBox (strBody)
'Print Output
    strInfo = strInfo & strBody
    PrintAisv (strInfo)
End Sub
...
Function GetEncore()
    ...
    If hasEncore <> "ERROR NO MATCH FOUND" Then
        RetStr = RetStr & ChrW(&H2612) & " <u>Encore playback</u><br>"
        ...
        If reportRequest = vbYes Then
            RetStr = RetStr & ChrW(&H2612) & "<u>Encore Report</u><br>"
...
Function GetMailingInfo()
    RetStr = ""
    RetStr = RetStr & ChrW(&H2612) & " <u>CD Record</u><br>"
    ' Record format:
    ...
    RetStr = RetStr & "&nbsp;&nbsp;&nbsp;&nbsp;<b>Qty:</b>" & qty & "<br>"
    'Shipping method
    shipMethod = RunGeneralRegex("Shipping Method: ([0-9a-zA-Z -]+)", TxtDataSheet.Text)
    RetStr = RetStr & "<b>Shipping Method</b>: " & shipMethod & "<br>" ' TODO: FIX SO THERE ARE BOXES WITH OPTIONS.
    ...
    RetStr = RetStr & "<b>Mailing Address</b>: " & mailAddr & "<br>"
    RetStr = RetStr & "<b>City:</b> " & mailCity & "&nbsp;&nbsp;&nbsp;&nbsp;<b>State/Province:</b> " & mailState & "<br>"
    RetStr = RetStr & "<b>Zip/Postal Code:</b> " & mailZip & "<br><br><br>"
...
Function GetFTPInfo()
    RetStr = ""
    RetStr = RetStr & ChrW(&H2612) & " <u>FTP Upload</u><br>"
    ' Record Format
...
Function GetAPL()
    ...
    If isAPL <> "ERROR NO MATCH FOUND" Then
        ...
        If Reply = vbYes Then
            RetStr = RetStr & "&nbsp;&nbsp;&nbsp;&nbsp;" & ChrW(&H2612) & "   Must be on list to join call<br>"
            'RetStr = RetStr & "&nbsp;&nbsp;&nbsp;&nbsp;" & ChrW(&H2610) & " Q&A Session Only<br>"
        Else
...
Function GetPolling()
    ...
    If isPolling <> "ERROR NO MATCH FOUND" Then
        RetStr = ChrW(&H2612) & " <u>Polling - Questions/Responses:</u><br>"
...
Function GetSpecialAnn()
    ...
    If isAnnunciator <> "ERROR NO MATCH FOUND" Then
        RetStr = RetStr & ChrW(&H2612) & " <u>Special Annunciator</u>"
...
Function GetSpecialScript()
    ...
    If isSpecialScript <> "ERROR NO MATCH FOUND" Then
        RetStr = RetStr & ChrW(&H2612) & " <u>Special Script</u> "
...
Function GetPartReport()
    ...
    If factsComplete <> "ERROR NO MATCH FOUND" Then
        RetStr = RetStr & ChrW(&H2612) & " <u>Participant report</u>"
        gatheredInfo = InputBox(Prompt:="Please enter the information gathered.", Title:="Facts complete information", Default:="First and Last Name")
...
Function GenerateCallType()
    ...
    If IS_QA <> "ERROR NO MATCH FOUND" Then
        ...
        RetStr = RetStr & "<ul><li><span style='background-color:yellow'>Will there be One Q&A session near the end of the Call or multiple Q&As throughout the call?</span>" & Space(1) & "</ul>"
...
Function GenerateDialInNums()
    Dim blEpPasscode, blEpTollFree, blEpToll, blOaPassword, blOaTollFree, blOaToll As Boolean
    blEpPasscode = False
    blEpTollFree = False
    blEpToll = False
    blOaPassword = True
...
'EP Participant:
    TollFreeEPPart = RunGeneralRegex("EP Participant (\d+)", TxtDataSheet.Text)
    If TollFreeEPPart <> "ERROR NO MATCH FOUND" Then
        TollFreeEPPasscode = TollFreeEPPart
...
' ITFS Numbers/Participants will reference:
    ...
    If blOaTollFree Or blOaToll Then
        If Password = "NO PASSWORD FOUND" Then
            ...
            If Reply = vbYes Then
                PartRef = "Conference ID Number<br>"
            Else
                PartRef = InputBox(Prompt:="What will Participants reference to join the call?", Title:="Participant reference entry", _
                                        Default:="Topic? etc....")
            End If
        End If
    End If
    'Conference Entry method
    RetStr = RetStr & "</ul><br><b>To join the conference:</b><br><ul>"
    'Leader reference
    RetStr = RetStr & "<li><b>Leaders will reference:</b> "
    If Password <> "NO PASSWORD FOUND" Then
        RetStr = RetStr & "PASSWORD - <span style='background-color:yellow'>" & Password & "</span>" & Space(1)
    ElseIf Reply = vbNo Then
        RetStr = RetStr & "OTHER: " & otherStr
    ElseIf Passcode <> "ERROR NO MATCH FOUND" Then
        RetStr = RetStr & "PASSCODE - <span style='background-color:yellow'>" & Passcode & "</span>" & Space(1)
    Else
        RetStr = RetStr & "Conference ID Number"
    End If
    RetStr = RetStr & "<br>"
    'EP Part Reference
    If blEpTollFree Or blEpToll Then
        RetStr = RetStr & "<li><b>EP Toll Free/Toll Participants will reference:</b> "
        If Passcode <> "ERROR NO MATCH FOUND" Then
            blEpPasscode = True
            RetStr = RetStr & "PASSCODE - <span style='background-color:yellow'>" & Passcode & "</span>" & Space(1)
        Else
            RetStr = RetStr & "NO PASSCODE - DIRECT ENTRY"
        End If
    End If
    'OA Part Reference
    If blOaTollFree Or blOaToll Then
        RetStr = RetStr & "<li><b>Participants will reference:</b> "
        If Password <> "NO PASSWORD FOUND" Then
            RetStr = RetStr & "PASSWORD - <span style='background-color:yellow'>" & Password & "</span>" & Space(1)
            RetStr = RetStr & "<ul><li><span style='background-color:yellow'>Will participants without the password still be accepted using alternative verification? (i.e. Conference ID/Company Name)</span>" & Space(1) & "</ul>"
        Else
            RetStr = RetStr & PartRef
        End If
        'RetStr = RetStr & "</ul>"
    End If
    'ITFS Part Reference
    If ITFSUsed = "Yes" Then
        RetStr = RetStr & "<li><b>ITFS Participants will reference:</b> "
        If blEpTollFree Or blEpToll Then
            RetStr = RetStr & "PASSCODE - "
            If blEpPasscode Then
                RetStr = RetStr & Passcode
            Else
                If blEpTollFree Then
                    RetStr = RetStr & TollFreeEPPasscode
                End If
                If blEpTollFree And blEpToll Then
                    RetStr = RetStr & " <b>or</b> "
                End If
                If blEpToll Then
                    RetStr = RetStr & TollEPPasscode
                End If
            End If
            RetStr = RetStr & "</ul>"
        ElseIf blOaTollFree Or blOaToll Then
            If Password <> "NO PASSWORD FOUND" Then
                RetStr = RetStr & "PASSWORD - <span style='background-color:yellow'>" & Password & "</span>" & Space(1)
            Else
                RetStr = RetStr & "Conference ID Number"
...
Function GenerateReferences()
    ...
    RetStr = "<b>To join the conference:</b><br><ul>"
    RetStr = RetStr & "<li><b>Leaders will reference:</b> "
    RetStr = RetStr & "<li><b>EP Toll-Free/Toll Participants will reference:</b> "
    RetStr = RetStr & "<li><b>Toll-Free/Toll Participants will reference:</b> "
    RetStr = RetStr & "<li><b>ITFS Participants will reference:</b> "
    GenerateReferences = RetStr
End Function
...
Function GetCommList()
    ...
    Exit Function
ErrHandler:
    GetCommList = "<span style='background-color:yellow'>Will you be using?</span>" & Space(1) & "<br>" & Space(1)
End Function
...
Function ReturnUserName()
    ...
    Else
        ...
        End If
    End If
    strUserName = StrConv(LCase(strUserName), vbProperCase)
...
'https://stackoverflow.com/a/15723794
Function AlphaNumericOnly(ByVal strSource As String) As String
    Dim i As Integer
...
Sub PrintAisv(strOutput As String)

    Dim FileNum As Integer
    Dim strIn, strFilePath As String
    
    strIn = strOutput
    strFilePath = "C:\Temp\AISV.TXT"
    
    FileNum = FreeFile ' next free filenumber
    
    Open strFilePath For Output As #FileNum ' creates the new file
    
    Print #FileNum, strIn
    Close #FileNum ' close the file
    
    intResult = Shell("Notepad.exe " & strFilePath, vbNormalNoFocus)
    Kill strFilePath
